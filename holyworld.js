...
function renderRows(rows) {
  ...
  tbody.innerHTML = rows.map(r => {
    ...
    return `<tr>
      <td><div class="td-server"><img src="holyworld.png" class="td-server-icon" alt="" onerror="this.style.display='none'"><span class="td-server-name">${esc(r.server || 'HolyWorld')}</span></div></td>
...
function archiveNicks(rows) {
  try {
    ...
    for (const r of rows || []) {
      ...
      if (!nick) continue;
      const existing = archive.find(e => e.nick === nick && e.server === (r.server || 'HolyWorld'));
      if (existing) {
        // Обновляем только если поменялся донат
        if (r.privilege && existing.donate !== r.privilege) existing.donate = r.privilege;
      } else {
        archive.push({ nick, donate: r.privilege || '', server: r.server || 'HolyWorld', ts: now });
        added++;
      }
    }
...
function flushExpiredToTxt(expired) {
  try {
    ...
    for (const e of expired) {
      const server = e.server || 'HolyWorld';
...
async function loadFilters() {
  try {
    ...
    // Если текущий сервер (HolyWorld) есть в списке — отметим его выбранным
    const cur = serverMenu.querySelector(`.ft-option[data-value="${esc(state.server)}"]`);
